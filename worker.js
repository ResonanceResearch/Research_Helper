export default {
  async fetch(request, env, ctx) {
    ...
    if (pathname === "/api/submit" && request.method === "POST") {
      // Optional: store anonymized session
      try {
...
function extractIdentity(context){
  ...
  if (!idQ) return null;
  // Allow "Jane Doe — University of Calgary, UCVM"
...
Personalize using prior answers and OpenAlex concepts if provided.`;
  ...
  try {
    ...
    // Expect { chips: [ ... ] } or a simple array
    if (out && Array.isArray(out.chips)) return out.chips;
    if (out && Array.isArray(out)) return out;
  } catch {}
  // Fallback heuristics
  const fallbacks = [
    "Pilot study", "Local collaborator", "Internal grant", "UCVM facility", "Co‑authorship map", "Method clinic"
  ];
...
- Keep it concise and skimmable with short bullets.`;
  ...
  try {
    ...
    // Fallback plain text
    return [
      "90‑Day Action Plan (fallback)",
      "1) Define 1–2 focused project questions",
      "2) Meet 3 collaborators identified via UCVM Dashboard",
      "3) Submit 1 internal seed grant; prep external LOI",
      "4) Secure data/ethics; book facility time",
      "5) Weekly cadence: collaborator outreach, pilot data, manuscript outline"
    ].join("\n");
  }
}
