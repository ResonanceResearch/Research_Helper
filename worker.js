export default {
  async fetch(request, env, ctx) {
    ...
    if (pathname === "/api/health") {
      return json({ ok: true, has_kv: !!env.ANSWERS, has_openai: !!env.OPENAI_API_KEY, model: env.OPENAI_MODEL || "gpt-5-mini", chip_model: env.CHIPS_MODEL || "gpt-5-nano" }, cors);
    }
    ...
    if (pathname === "/api/diag") {
      const checks = {
        ...
        chip_model: env.CHIPS_MODEL || "gpt-5-nano"
      };
...
function hashKey(s){
  ...
  let h = 0x811c9dc5;
  for (let i=0;i<enc.length;i++){ h ^= enc[i]; h += (h<<1) + (h<<4) + (h<<7) + (h<<8) + (h<<24); }
...
async function suggestChips(questionText, context, openalex, env){
  ...
  // KV cache key: question + compact context (last 5 answers/chips)
  const compact = JSON.stringify({
    q: (questionText||"").trim(),
    c: (Array.isArray(context) ? context.slice(-5) : [])
          .map(x => ({ id: x.id, a: (x.answer||"").slice(0,200), ch: (x.chips||[]).slice(0,5) }))
  });
  const cacheKey = "chips:v2:" + hashKey(compact);

  if (env.ANSWERS) {
    try {
      const found = await env.ANSWERS.get(cacheKey, "json");
...
Return JSON: {"chips":["...","..."]}`;
  ...
  const chipModel = env.CHIPS_MODEL || "gpt-5-nano";
  let out = null;
  try {
    out = await withTimeout(callOpenAIWithOpts(prompt, env, { model: chipModel, jsonWanted: true, max_output_tokens: 80 }), 1500);
  } catch (e) {
    try { out = await callOpenAIWithOpts(prompt, env, { model: env.OPENAI_MODEL || "gpt-5-mini", jsonWanted: true, max_output_tokens: 80 }); } catch {}
  }
  ...
  if (out && Array.isArray(out.chips)) {
    ...
    const uniq = Array.from(new Set(list.map(c => c.trim()).filter(Boolean))).slice(0, 12);
    if (env.ANSWERS) { try { await env.ANSWERS.put(cacheKey, JSON.stringify(uniq), { expirationTtl: 86400 }); } catch {} }
    return uniq;
  }

  if (out && Array.isArray(out)) {
    const uniq = Array.from(new Set(out.map(c => c.trim()).filter(c => relevantToQuestion(questionText, c)))).slice(0, 12);
    if (uniq.length) {
      if (env.ANSWERS) { try { await env.ANSWERS.put(cacheKey, JSON.stringify(uniq), { expirationTtl: 86400 }); } catch {} }
      return uniq;
    }
  ...
  const uniqFallback = Array.from(new Set(fallbacks.filter(c => relevantToQuestion(questionText, c)))).slice(0, 12);
  if (env.ANSWERS) { try { await env.ANSWERS.put(cacheKey, JSON.stringify(uniqFallback), { expirationTtl: 43200 }); } catch {} }
  return uniqFallback;
}
...
async function callOpenAIWithOpts(messages, env, { model, jsonWanted=false, max_output_tokens=null } = {}){
  const m = model || env.OPENAI_MODEL || "gpt-5-mini";
