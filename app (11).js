...
function renderQuestion(){
  try{
    ...
    // NEW: If this is the last question, repurpose Finish -> Generate Plan
    ...
    // Clear any previous chips immediately to avoid stale suggestions
    renderChips([]);
...
async function exportPlan(){
  ...
  try{
    ...
    try { (els.actionPlan || document.querySelector(".plan"))?.scrollIntoView({ behavior: "smooth", block: "start" }); } catch {}
    save();
...
function finishInterview(){
  ...
  if (els.finish) { els.finish.textContent = "Generate Plan"; els.finish.onclick = generatePlan; }
  try {
    generatePlan().then(() => { (els.actionPlan || document.querySelector(".plan"))?.scrollIntoView({ behavior: "smooth", block: "start" }); });
  } catch (e) {
    (els.actionPlan || document.querySelector(".plan"))?.scrollIntoView({ behavior: "smooth", block: "start" });
  }
}
...
// NEW: Enter/Return submits the current answer
