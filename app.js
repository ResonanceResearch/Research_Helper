/* Faculty Research Mentor – corrected (with chip preloading & caching) */
...
function shouldFetchChips(q, i){
  // Skip chips for first question and for any question explicitly marked as no_chips
  if (!q) return false;
  if (i === 0) return false;
...
function renderQuestion(){
  try{
    ...
    // === NEW: instant render of cached chips; otherwise fetch if appropriate ===
    ...
    if (cached) {
      ...
      suggestChips(q)
        ...
        .catch(()=> renderChips([]))
        .finally(()=>{ if (els.qSpin) els.qSpin.classList.remove("show"); setInputsDisabled(false); });
    } else {
      renderChips([]);
    }
    ...
    // === NEW: prefetch chips for upcoming questions ===
    prefetchAhead(idx, 2);
  }catch(e){
...
async function suggestChips(q){
  try{
    ...
    // Limit context to the most recent 5 answers to keep payloads small and faster
    ...
    const chips = Array.isArray(data.chips) ? data.chips.slice(0, 12) : [];
    return chips;
  }catch(e){
...
(async function init(){
  ...
  // === NEW: prefetch chips early (skip index 0 by shouldFetchChips rule) ===
  prefetchAhead(-1, 3);
