...
let RESOURCES = [];
let idx = 0;
let PREFETCHED_NEXT = null; // prefetch slot
...
function renderQuestion(){
  try{
    ...
    if (idx === 0) {
      ...
      withTimeout(suggestChips(q), 1200, [])
        .then(chips => renderChips(chips))
        .catch(() => renderChips([]))
        .finally(() => {
          if (els.qSpin) els.qSpin.classList.remove("show");
          setInputsDisabled(false);
    ...
    if (typeof renderQuestionsPreview === "function") renderQuestionsPreview();
...
async function prefetchNextQuestion(){
  if(!window.API_BASE) return;
  const context = (STATE.answers || []).slice(-5).map(a => ({
    id: a.id, answer: a.text, chips: a.chipsAccepted || []
  }));
  try{
    ...
    if (res.ok) {
      const data = await res.json();
      if (data && data.id && data.text) PREFETCHED_NEXT = data;
    }
  } catch {}
}
...
(function(){
  ...
  submitAnswer = function(){
    ...
    if (idx < QUESTIONS.length - 1){
      ...
      if (PREFETCHED_NEXT && PREFETCHED_NEXT.id && PREFETCHED_NEXT.text) {
        QUESTIONS.push({
          ...PREFETCHED_NEXT,
          required: false,
          weight: 3,
        ...
        if (typeof finishInterview === "function") finishInterview();
      }
    }
  };
})();
